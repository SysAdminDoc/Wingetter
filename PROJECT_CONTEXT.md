...
- Local validation: `tools\Invoke-Validation.ps1` runs catalog sync/count validation, profile JSON tests, profile gallery tests, WinGet runner helper tests (including locale-independent classification fixtures under `tools\fixtures\winget\`), search metadata tests, package-source adapter tests, source policy tests, update watcher tests, offline cache tests, WinGet Configuration export tests, visual/accessibility tests, release artifact hash verification, launcher manifest verification, bundle validation, WPF XAML load validation, and PSScriptAnalyzer on Windows.
...
- The checked-in `Wingetter.exe` is hash-pinned via `release\manifest.json`, and `tools\Build-WingetterExe.ps1` concatenates the dot-sourced modules into a parser-clean bundled launcher (PS2EXE packaging is opt-in via `-RunPS2EXE`). `tools\Test-Bundle.ps1` exercises the builder through local validation so module-list drift or concatenation errors fail before a release. Release builds are produced locally, `tools\Test-ReleaseArtifact.ps1 -Update` refreshes the manifest, and `tools\Invoke-Validation.ps1` verifies the result before commit/push.
...
- Catalog JSON, embedded module fallback sync, validation tooling, and the local validation runner now exist.
- Local validation now covers catalog sync/counts, profile JSON helpers imported from modules, profile gallery hashes and catalog references, WinGet runner helpers imported from modules, source adapter boundaries, source policy behavior, update watcher classification, offline cache manifests, WinGet Configuration export generation, search metadata, visual/accessibility checks, release artifact hashes, launcher module hashes, bundle generation, analyzer checks, and XAML loading. Full GUI automation remains a roadmap item.
