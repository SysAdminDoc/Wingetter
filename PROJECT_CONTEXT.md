...
- CI workflow: `.github/workflows/validate.yml` runs catalog sync/count validation, profile JSON tests, WinGet runner helper tests, search metadata tests, package-source adapter tests, visual/accessibility tests, and WPF XAML load validation on Windows.
...
- WinGet install/update operations launch `winget install` or `winget upgrade` with `--id`, `--exact`, and optional `--silent` / agreement flags.
...
- CI now covers catalog sync/counts, profile JSON helpers imported from modules, WinGet runner helpers imported from modules, source adapter boundaries, search metadata, visual/accessibility checks, and XAML loading. There is still no release build script or full GUI automation.
...
1. Add corporate/internal source mode on top of the new adapter boundary, including allowed-source policy, explicit source metadata, and audit export.
