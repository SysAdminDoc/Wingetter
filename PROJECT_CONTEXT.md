...
- Source modules: `src/Wingetter.Common.ps1`, `src/Wingetter.Catalog.ps1`, `src/Wingetter.WinGet.ps1`, `src/Wingetter.Groups.ps1`, `src/Wingetter.Sources.ps1`, `src/Wingetter.UpdateWatcher.ps1`, `src/Wingetter.Ui.ps1`, and `src/Wingetter.App.ps1`.
...
- CI workflow: `.github/workflows/validate.yml` runs catalog sync/count validation, profile JSON tests, WinGet runner helper tests, search metadata tests, package-source adapter tests, source policy tests, update watcher tests, visual/accessibility tests, and WPF XAML load validation on Windows.
...
- CI now covers catalog sync/counts, profile JSON helpers imported from modules, WinGet runner helpers imported from modules, source adapter boundaries, source policy behavior, update watcher classification, search metadata, visual/accessibility checks, and XAML loading. There is still no release build script or full GUI automation.
...
1. Add offline download/cache mode for air-gapped or low-bandwidth rebuild workflows.
