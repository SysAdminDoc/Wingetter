...
- Preflight run-plan gap addressed after this research pass: install/update clicks now create a JSON-backed plan with per-package action/status/reason/source/pin/version fields, the review dialog removes blocked/current/unresolved rows before execution, and migration reports embed the plan.

## Architecture Assessment
- `src/Wingetter.Ui.ps1` remains the dominant module, but the install/update/offline run-loop gap is addressed after this research pass: package operations now run in a background runspace, progress/log messages are marshaled back through a dispatcher timer, and cancellation uses a shared token instead of `DoEvents` pumping.
