...
- [ ] P0 — Restore a single validation contract
  Why: Current validation is not green because the README omits the `v6.1.0` badge, and release docs still reference removed GitHub workflows plus a "future" build script that already exists.
  Evidence: `tools\Test-Catalog.ps1` failure; `README.md:5-8`; `release/README.md:15-16`; `release/README.md:41-45`; commit `2d285fb`.
  Touches: `README.md`, `release/README.md`, `tools\Invoke-Validation.ps1` (new), `tools\Test-Catalog.ps1`.
  Acceptance: one local validation command runs catalog/profile/gallery/runner/search/source/update/offline/configuration/accessibility/release/launcher/bundle/XAML/analyzer checks and exits zero; docs contain no stale `.github/workflows/*` claims.
  Complexity: S
