...
- Build: embed a `$Script:WingetterModuleHashes` hashtable in `Wingetter.ps1` keyed by module file name with the canonical SHA256 of each module under `src\`; verify each downloaded module's SHA256 against the table after staging and before dot-sourcing; refuse to dot-source on any mismatch with a precise error message. Add `tools\Sync-LauncherManifest.ps1` that regenerates the embedded hashtable from the current `src\` tree, and `tools\Test-LauncherManifest.ps1` that fails CI when the embedded hashtable is out of sync with the modules so a maintainer cannot land a module change without bumping the launcher table at the same time. Wire the test into `.github/workflows/validate.yml`.
- Acceptance: `tools\Test-LauncherManifest.ps1` exits zero against the current tree, exits nonzero if any `src\Wingetter.*.ps1` file's SHA256 differs from the embedded launcher table, and the launcher refuses to dot-source a downloaded module whose hash differs from the embedded value.
- Sources: PROJECT_CONTEXT.md "Strategic Direction" item 3.
- Completed 2026-05-18: added `$Script:WingetterModuleHashes` hashtable to `Wingetter.ps1` between explicit `# BEGIN WingetterModuleHashes` / `# END WingetterModuleHashes` markers (so the sync tool can replace the block deterministically), and a `Test-WingetterModuleHash` helper that the staged-file path now calls before `Move-Item` so a hash mismatch refuses the download instead of dot-sourcing a tampered module. Added `tools\Sync-LauncherManifest.ps1` that reads the canonical module list from `$Script:WingetterModuleFiles`, recomputes each hash from `src\`, and replaces the marked block (idempotent: re-running on a current launcher prints "already current"). Added `tools\Test-LauncherManifest.ps1` that verifies (a) the embedded hashtable matches every module on disk, (b) the hashtable contains exactly the declared module list, and (c) the verifier function itself accepts a canonical module and refuses a deliberately tampered one. Wired the test into both `.github/workflows/validate.yml` and `.github/workflows/release.yml` so pre-release builds also catch drift.

### [x] R-028 - Tag-triggered PS2EXE release build

- Problem: `tools\Build-WingetterExe.ps1 -RunPS2EXE` exists and produces a fresh `Wingetter.exe` from the modular launcher, but it has to be run manually on a Windows host with PS2EXE installed. There is no CI path that produces an EXE artifact when a release tag is pushed, so the checked-in binary lags the modular code and contributors have no canonical place to download a fresh build matching a tag.
- Build: add `.github/workflows/release.yml` that triggers on push of a `v*.*.*` tag, installs the PS2EXE module on a fresh `windows-latest` runner, runs `tools\Build-WingetterExe.ps1 -RunPS2EXE`, verifies the bundle parses, uploads the resulting `Wingetter.exe` and the regenerated `release\manifest.json` as workflow artifacts, and attaches both files to a GitHub Release for the tag. The workflow must not push back to `main`; manifest updates remain a maintainer decision.
- Acceptance: pushing a tag like `v6.2.0` on GitHub produces a workflow run that successfully builds `Wingetter.exe`, uploads it as a release asset, and exits zero. The on-push CI surface is unchanged.
- Sources: PROJECT_CONTEXT.md "Strategic Direction" item 3 (updated).
- Completed 2026-05-18: added `.github/workflows/release.yml` triggered on `push` of a `v*.*.*` tag (and on `workflow_dispatch` for ad-hoc invocations). The job checks out the repo, resolves the release tag from either the workflow input or `github.ref`, runs the same 13 pre-release validation scripts the regular CI runs (bundle and analyzer included), installs PS2EXE from PSGallery if not already present, builds `Wingetter.exe` via `tools\Build-WingetterExe.ps1 -RunPS2EXE`, regenerates `release\manifest.json` with `tools\Test-ReleaseArtifact.ps1 -Update`, sanity-checks that the manifest verifies against the freshly built binary, uploads `Wingetter.exe` + `release\manifest.json` + `release\Wingetter.bundled.ps1` as workflow artifacts, and (only when the run is on an actual tag, not a manual dispatch without one) attaches `Wingetter.exe` and `release\manifest.json` to a GitHub Release via `softprops/action-gh-release@v2`. The workflow has `contents: write` permission for the release publish step but never pushes back to `main` - manifest updates remain a maintainer decision because the maintainer has to manually commit the regenerated manifest if they want to advance the tracked hash.

### [x] R-027 - Automated keyboard navigation and accessibility validation

- Problem: `tools\Test-VisualAccessibility.ps1` only checks two specific automation names (theme toggle, search box) and the rectangular-radius rule. Every other named focusable control - 30+ buttons, the group combo, the gallery list, the package details panel - is unverified. A future commit could drop `AutomationProperties.Name=` from the theme toggle and re-add a different control without an accessible name, and CI would not notice. PROJECT_CONTEXT.md still lists "deeper keyboard navigation and screen reader flow testing are still manual" as an open gap.
- Build: extend `tools\Test-VisualAccessibility.ps1` so it walks every `<Button|TextBox|ComboBox|CheckBox|ToggleButton|ListBox|RadioButton>` element that carries an `x:Name=` attribute and asserts each one has at least one accessible label source (`Content=`, `Text=`, `AutomationProperties.Name=`, or `ToolTip=`); textless controls (`TextBox`, `ComboBox`, `ListBox`) must have `AutomationProperties.Name=` explicitly because they have no inherent label. Also forbid `Focusable="False"` and `IsTabStop="False"` on any user-facing named control (the one WPF template-part exception, `<ToggleButton x:Name="ToggleButton" Template=...>` inside the `ComboToggle` style, is allow-listed by checking for `Template=`).
- Acceptance: `tools\Test-VisualAccessibility.ps1` continues to exit zero against the current tree and exits nonzero if a future commit adds a named focusable control without an accessible label, or sets `Focusable=False`/`IsTabStop=False` on a user-facing control.
- Sources: PROJECT_CONTEXT.md "Strategic Direction" item 3.
- Completed 2026-05-18: extended `tools\Test-VisualAccessibility.ps1` to enumerate every named focusable control via regex, allow-list template parts (controls carrying a `Template=` attribute), require an accessible label source on each control (with the stronger `AutomationProperties.Name=` rule on textless `TextBox`/`ComboBox`/`ListBox`), reject `Focusable="False"` / `IsTabStop="False"` outside template parts, and floor the sweep at 20 controls so a regex regression that drops every match cannot silently pass. The first run surfaced four real gaps - `GroupCombo`, `GroupNameBox`, `ProfilesList`, `PreviewBox` - all now carry descriptive `AutomationProperties.Name=` values ("Saved package groups", "Group name", "Available profile gallery profiles", "Profile package review").
